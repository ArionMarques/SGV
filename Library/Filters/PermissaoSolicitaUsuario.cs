...
{
    public class PermissaoSolicitaUsuario : ActionFilterAttribute
    {
        ...
        public override void OnActionExecuting(ActionExecutingContext context)
        {
            ...
            if (conexao.State == System.Data.ConnectionState.Closed)
            {
                ...
                reader = new MySqlCommand("select permissoes.id from permissoes inner join menus on permissoes.fk_menu = menus.id inner join sub_menus on permissoes.fk_sub_menu = sub_menus.id where fk_Operador =" + Operador + " and Sub_Menu ='" + "Solicitação de acesso" + "'", command.Connection).ExecuteReader();
                reader.Read();
                if (reader.HasRows)
                {
                    IsValid = true;
